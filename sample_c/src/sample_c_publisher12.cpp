...
class vel_cal{
 ...
 void vel_cal::chatterCallback(const visualization_msgs::Marker& marker){
  ...
  for(i = 0; i < cnt; i++){
   if(marker.pose.position.z > 0.7 && marker.pose.position.z < 0.9){
    twist.linear.x = 0.0;
   }else if(marker.pose.position.z > 0.9){
        twist.linear.x = 0.1;
    if(marker.pose.orientation.z > 0.2 || marker.pose.position.x < -0.1){
     twist.angular.z = 0.25;
    }else if(marker.pose.orientation.z < -0.2 || marker.pose.position.x > 0.1){
     twist.angular.z = -0.25;
    }
   }else{
    twist.linear.x = -0.1;
